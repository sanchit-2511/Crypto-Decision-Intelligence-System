...
run_pipeline.py
---------------
Master pipeline orchestrator — Version 3.
Flow: Fetch → Clean → Store → Analyse → Signals → Insights → Dashboard
...
def run_pipeline():
    print(f"\n{'='*55}")
    print(f"  🚀 Pipeline v3 @ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*55}")
    ...
    # Step 2: Clean (with health report)
    ...
    # Step 4: Analyse + Signals
    print(f"\n  🔬 Running indicators...")
    signals_df = generate_signals()
    store_signals(signals_df)

    # Step 5: Generate insights + market summary
    print(f"\n  🧠 Generating insights...")
    prices_df = get_latest_prices()
    enriched  = generate_coin_insights(signals_df)
...
def main():
    parser = argparse.ArgumentParser(description="Crypto Intelligence Pipeline v3")
